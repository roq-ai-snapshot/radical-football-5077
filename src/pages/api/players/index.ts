...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createPlayer() {
    ...
    const body = { ...req.body };
    if (body?.player_performance?.length > 0) {
      const create_player_performance = body.player_performance;
      body.player_performance = {
        create: create_player_performance,
      };
    } else {
      delete body.player_performance;
    }
